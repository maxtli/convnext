...
class GoogLeNet(nn.Module):
    ...
    def patch(self, x, alts, bsz, device):
        eye = torch.eye(x.shape[1]).to(device)
        replacement_dims = einsum("filt o_filt, N filt H W -> N o_filt filt H W", eye, alts[0]-x[:bsz])
        patches = (x[:bsz].unsqueeze(1) + replacement_dims).flatten(0,1)
        x = torch.cat([x,patches], dim=0)
        return x

    def _forward(self, x: Tensor, partial=None, alts=None) -> Tuple[Tensor, Optional[Tensor], Optional[Tensor]]:
        ...
        if self.partial == "primary":
            ...
            # N x 192 x 56 x 56
            x = self.maxpool2(x)
            x = self.patch(x,alts[0],bsz, device)

            # N x 192 x 28 x 28
            x = self.inception3a(x)
            x = self.patch(x,alts[1],bsz, device)
            # N x 256 x 28 x 28
            x = self.inception3b(x)
            x = self.patch(x,alts[2],bsz, device)

            # N x 480 x 28 x 28
            x = self.maxpool3(x)
            # N x 480 x 14 x 14
            x = self.inception4a(x)
            x = self.patch(x,alts[3],bsz,device)
            return self.remainder_fwd(x)
    ...
    def forward(self, x: Tensor, partial=None, alts=None) -> GoogLeNetOutputs:
        x = self._transform_input(x)
        x, aux1, aux2 = self._forward(x, partial, alts)
...
def googlenet_edit(*, weights: Optional[GoogLeNet_Weights] = None, progress: bool = True, **kwargs: Any) -> GoogLeNet:
